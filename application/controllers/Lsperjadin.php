...
class Lsperjadin extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('m_lsbendahara');
    ...
    function index()
    {
        ...
        $data['title'] = 'LS Bendahara - Perjalanan Dinas';

        $data['menu'] = $this->db->get('master_modules')->result_array();

        $data['js'] = "lsprj.js";

        $this->load->model('m_lsbendahara', 'lsbendahara');

        $data['xls'] = $this->db->get('spp')->result_array();
        $data['Lsp'] = $this->lsbendahara->getlspBy($this->session->userdata('id'));

        if ($this->form_validation->run() == false) {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('spp/v_lsperjadin', $data);
            $this->load->view('templates/footer', $data);
        }
    }

    function ubah_lsperjadin()
    {
        ...
        foreach ($nospp as $key => $value) {
            ...
            $this->m_lsbendahara->up_lsperjadin($update, $dok);
    ...
    function simpan_lsperjadin()
    {
        ...
        foreach ($nospp as $key => $value) {
            date_default_timezone_set('Asia/Jakarta');
            $this->load->model('m_lsbendahara', 'kode');
            $kode_ls = $this->kode->kode_lsp();
            $dok = $kode_ls;
            $created = date('d-m-Y H:i:s');
            $tgl_pengajuan = date('d-m-Y');
            $created_by = $this->session->userdata['id'];
            ...
            $this->m_lsbendahara->ins_lsperjadin($insert);
    ...
    function hapus_lsperjadin()
    {
        date_default_timezone_set('Asia/Jakarta');

        $dokumen_id = $this->input->post('dkode_ls');
        ...
        $this->m_lsbendahara->hapus_lsperjadin($dokumen_id, $deleted, $deleted_by);
        $this->session->set_flashdata('messege', '<div class="alert alert-warning" role="alert">Data has been deleted!</div>');
        redirect('lsperjadin');
    }

    function getLsPerjadin()
    {
        $this->load->model('m_lsbendahara', 'xlsprj');
        $id = $_GET['id'];
        $data = $this->xlsprj->getLs($id);
