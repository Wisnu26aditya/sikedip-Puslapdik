...
class Tup extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['title'] = 'Tambahan Uang Persediaan';

        $data['menu'] = $this->db->get('master_modules')->result_array();

        $data['js'] = 'tup.js';

        $this->load->model('m_tup', 'xtup');


        //$data['list_tup'] = $this->db->get('dokumen_lpj')->result_array();
        $data['Tup'] = $this->xtup->getTup($this->session->userdata['id']);

        if ($this->form_validation->run() == false) {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('lpj/v_tup', $data);
    ...
    function simpan_tup()
    {
        ...
        $this->load->model('m_tup', 'kode');
        $kode_tup = $this->kode->kode_tup();
        $dokumen_id = $kode_tup;
    ...
    function ubah_tup()
    {
        ...
        $tglsp2d = $this->input->post('tglsp2d');
        $nilaisp2d = $this->input->post('nilaisp2d');
        $dokumen_id = $this->input->post('kode_tup');
        ...
        $this->m_tup->up_tup($update, $dokumen_id);
    ...
    function hapus_tup()
    {
        ...
        $dokumen_id = $this->input->post('dkode_tup');
        ...
        $this->m_tup->hapus_tup($dokumen_id, $deleted, $deleted_by);
