...
class Honkeg extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model('m_lpj');
        $this->load->model('m_honkeg');
    ...
    function index()
    {
        ...
        $data['title'] = 'Honor Kegiatan';

        $data['menu'] = $this->db->get('master_modules')->result_array();

        $data['js'] = 'hkg.js';

        $this->load->model('m_honkeg', 'xhonkeg');

        //$data['list_honkeg'] = $this->db->get('dokumen_lpj')->result_array();

        $data['Honkeg'] = $this->xhonkeg->getHonkeg($this->session->userdata['id']);

        if ($this->form_validation->run() == false) {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('lpj/v_honkeg', $data);
            $this->load->view('templates/footer', $data);
        }
    }

    function simpan_honkeg()
    {
        ...
        $this->load->model('m_honkeg', 'kode');
        $kode_hkg = $this->kode->kode_hkg();
        $dokumen_id = $kode_hkg;
    ...
    function ubah_honkeg()
    {
        ...
        $tglsp2d = $this->input->post('tglsp2d');
        $nilaisp2d = $this->input->post('nilaisp2d');
        $dokumen_id = $this->input->post('kode_hkg');
        ...
        $this->m_honkeg->up_honkeg($update, $dokumen_id);
        $this->session->set_flashdata('messege', '<div class="alert alert-success" role="alert">Data Updated!</div>');
        redirect('honkeg');
    }

    function hapus_honkeg()
    {
        date_default_timezone_set('Asia/Jakarta');

        $dokumen_id = $this->input->post('dkode_hkg');
        ...
        $this->m_honkeg->hapus_honkeg($dokumen_id, $deleted, $deleted_by);
        $this->session->set_flashdata('messege', '<div class="alert alert-warning" role="alert">Data has been deleted!</div>');
        redirect('honkeg');
    }

    function getHonkegId()
    {
        $this->load->model('m_honkeg', 'xhonkeg');
        $id = $_GET['id'];
        $data = $this->xhonkeg->getHonkegId($id);
