...
class Pbjatas50 extends CI_Controller
{
    ...
    function index()
    {
        ...
        $data['title'] = 'Pengadaan Barang/Jasa 50 s.d 200 Juta';

        $data['menu'] = $this->db->get('master_modules')->result_array();

        $data['js'] = 'pbjatas50.js';

        $this->load->model('m_pbj', 'xpbj');

        //$data['list_pbj'] = $this->db->get('dokumen_lpj')->result_array();
        $data['Pbj'] = $this->xpbj->getPbj_d($this->session->userdata['id']);

        if ($this->form_validation->run() == false) {
            $this->load->view('templates/header', $data);
            $this->load->view('templates/sidebar', $data);
            $this->load->view('templates/topbar', $data);
            $this->load->view('lpj/v_pbjatas50', $data);
            $this->load->view('templates/footer', $data);
        }
    }


    function simpan_pbjatas50()
    {
        ...
        if (!empty($_FILES['file_pengadaan']['name']) || $_FILES['file_pengadaan']['name'] != "") {
            if ($this->upload->do_upload('file_pengadaan') && !empty($_FILES['file_pengadaan']['name'])) {
                ...
                $insert['dokumen_pengadaan'] = $file_pengadaan;
            }
        }
    ...
    function ubah_pbjatas50()
    {
        ...
        if (!empty($_FILES['file_pengadaan']['name']) || $_FILES['file_pengadaan']['name'] != "") {
            if ($this->upload->do_upload('file_pengadaan') && !empty($_FILES['file_pengadaan']['name'])) {
                $gbrfile_pengadaan = $this->upload->data();
                $file_pengadaan = $gbrfile_pengadaan['file_name'];
                $update['dokumen_pengadaan'] = $file_pengadaan;
            }
        }
        if (!empty($_FILES['file_karwas']['name']) || $_FILES['file_karwas']['name'] != "") {
            if ($this->upload->do_upload('file_karwas') && !empty($_FILES['file_karwas']['name'])) {
    ...
    function hapus_pbjatas50()
    {
        ...
        if ($file_pengadaan != null) {
            unlink('src/upload/lpj/' . $file_pengadaan);
        }
        if ($file_karwas != null) {
    ...
    function getPbjatas50Id()
    {
        $this->load->model('m_pbj', 'xpbj');
        $id = $_GET['id'];
        $data = $this->xpbj->getPbj_dId($id);
