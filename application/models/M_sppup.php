...
class M_sppup extends CI_Model
{
    public function getSppup($id)
    {
    ...
    public function getsppupBy($user)
    {
        $query = "SELECT 	a.spp_id,
                            a.spp_no,
                            a.spp_tgl,
                            ...
                            a.created_by
                    FROM spp a
                    WHERE a.spp_dok_id LIKE 'SPPU-%' 
                    and a.created_by = '" . $user . "' and a.show_item='1'
                    ORDER BY a.spp_id";
        return $this->db->query($query)->result_array();
    }

    public function kode_sppup()
    {
        ...
        $kodejadi = "SPPU-" . $datenow . $kodemax;    // hasilnya ODJ-9921-0001 dst.
        return $kodejadi;
    }

    function ins_sppup($data)
    {
        $this->db->insert('dokumen_spp', $data);
    }

    function up_sppup($data, $id)
    {
        $this->db->update('dokumen_spp', $data, array('spp_dok_id' => $id));
    }

    function hapus_sppup($dokumen_id, $deleted_date, $deleted_by)
    {
