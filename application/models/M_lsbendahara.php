...
class M_lsbendahara extends CI_Model
{
    public function getLs($id)
    {
    ...
    public function getlspBy($user)
    {
        $query = "SELECT 	a.spp_id,
                            ...
                            a.created_by,
                            CASE
                            WHEN a.show_item='1' THEN 'Diajukan'
                            WHEN a.show_item='2' THEN 'Terverifikasi'
                            ELSE 'tidak ada keterangan'
                            END status
                    FROM spp a
                    WHERE a.spp_dok_id LIKE 'LSP-%' 
                    and a.created_by = '" . $user . "' and a.show_item='1'
                    ORDER BY a.spp_id";
        return $this->db->query($query)->result_array();
    }

    public function kode_lsp()
    {
        ...
        $kodejadi = "LSP-" . $datenow . $kodemax;    // hasilnya ODJ-9921-0001 dst.
        return $kodejadi;
    }

    function ins_lsperjadin($data)
    {
        $this->db->insert('dokumen_spp', $data);
    }

    function up_lsperjadin($data, $id)
    {
        $this->db->update('dokumen_spp', $data, array('spp_dok_id' => $id));
    }

    function hapus_lsperjadin($dokumen_id, $deleted_date, $deleted_by)
    {
